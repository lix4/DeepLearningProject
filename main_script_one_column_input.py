...
from keras.layers.convolutional import Conv3D, MaxPooling2D, Conv2D
...
#x is input
#y is output
x_train = data_input[0:15000]
x_test = data_input[15000:20520]
y_train = data_output[0:15000]
y_test = data_output[15000:20520]
...
def BuildModel(hp):
    ...
    def BuildModule(model, depth=1, filters=16, input_flag=True):
        ...
        for j in np.arange(depth) + 1:
            if input_flag:
                model.add(Conv2D(filters, 3, strides=2,
                                 padding='same', input_shape=(240, 240, 3)))
                input_flag = False
            elif j == depth:
                filters = filters * 2
...
# y_train[:,1].isnan()
nanVals = np.argwhere(np.isnan(y_train))
num = np.isnan(y_train).sum()
for i in range(num):
    print(nanVals[i])
    first = nanVals[i][0]
    second = nanVals[i][1]
    print(y_train[first][second])
    y_train[first][second] = 0
#     print(y_train[nanVals[i]])
...
EPOCHS = 10
BATCH = 100
TRIALS = 25  # need to increase trials eventually
...
#     try:
    ...
    print('d1: {}, d2: {}, filters: {}, nodes: {}, stopping: {}, drop: {}'.format(hp.depth1, hp.depth2, hp.filters, hp.nodes, hp.stopping, hp.dropout))
